...
# i log ptp servono alla sezione 4, li metto da parte prima di fermare ptp4l
cp /tmp/ptp_repl.log /tmp/ptp_elim.log /tmp/ptp_talker.log /tmp/ptp_listener.log /tmp/tsn_test/
# ptp4l genera 8 messaggi/s per porta e sporca la misura
pkill ptp4l
sleep 1
...
echo ""
# senza un collo di bottiglia i due flussi non si contendono nulla e il gate non
# ha niente da proteggere: wlan2 e' l'unico punto del percorso in avanti dove un
# limite di banda agisce davvero (le veth FRER sono scritte da ndo_xdp_xmit, che
# scavalca il livello qdisc)
...
# senza un termine di paragone i numeri qui sopra non dicono se il gate serva
